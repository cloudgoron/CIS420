...
function printDriverListAsCarousel(list, car){
	...
	var txt = list[0].txt;
	// alert(car.list);
	var htmlStr = '<div class="item active"><img src="' + img + '" alt="..."><div class="carousel-caption"><p>'+txt+'<br/><form action="route.html" method = "get"><input type="hidden" name="carType" value="'+car.list+'"><input type="hidden" name="car" value="'+car.listIndex+'"><input type="hidden" name="driver" value="0"><input type="submit" value = "I want this one"></form></p></div></div>';
	// document.writeln(cars.length);
	for(var i = 1; i < list.length; i ++){
		img = list[i].img;
		txt = list[i].txt;
		htmlStr += '<div class="item"><img src="' + img + '" alt="..."><div class="carousel-caption"><p>'+txt+'<form action="route.html" method = "get"><input type="hidden" name="carType" value="'+car.list+'"><input type="hidden" name="car" value="'+car.listIndex+'"><input type="hidden" name="driver" value="'+i+'"><input type="submit" value = "I want this one"></form></p></div></div>';
	}
	return htmlStr;
};
...
function getAllFromURLForCheckout(){
	...
	var returnVal = {
		...
		dist: args[5].split("=")[1],
		price: args[6].split("=")[1]
	}
	return returnVal;
};


// function replacePlusSigns(str){
